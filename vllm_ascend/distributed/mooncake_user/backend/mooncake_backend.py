...
from vllm_ascend.distributed.mooncake_user.user_config_data import MooncakeStoreConfig
...
class MooncakeBackend(Backend):
    ...
    def get(self, key: str, addrs: list[int], sizes: list[int]):
        ...
        try:
            res = self.store.batch_get_into(key, addrs, sizes)
            if res[0] != expect_res:
    ...
    def put(self, key: str, addrs: list[int], sizes: list[int]):
        try:
            ret = self.store.batch_put_from(key, addrs, sizes)
            if ret[0] != 0:
